...
public class EmployeeApiController : BaseApiController
{
    ...
    public async Task<ActionResult<EmployeeResponse>> DeleteAsync(int id)
    {
        ...
        EmployeeResponse result = await _employeeService.DeleteAsync(id, cts.Token);
    ...
    {
        CancellationTokenSource cts = new();
        EmployeeResponse result = await _employeeService.FindEmployeeByIdAsync(id, cts.Token);
    ...
    public async Task<ActionResult<IEnumerable<EmployeeDto>>> ListAsync([FromQuery] PagingParameterModel? paging = null)
    {
        ...
        IEnumerable<EmployeeDto> employees = await _employeeService.GetEmployeesAsync(paging, cts.Token);
        return Ok(employees);
    }
    ...
    public async Task<ActionResult> PostAsync([FromBody] EmployeeDto employee)
    {
        ...
        CancellationTokenSource cts = new();
        EmployeeResponse result = await _employeeService.SaveAsync(employee, cts.Token);
    ...
    {
        CancellationTokenSource cts = new();
        EmployeeResponse result = await _employeeService.UpdateAsync(id, employee, cts.Token);
        if (!result.Success)
        {
            return BadRequest(new ErrorResource(result.Message));
        }
